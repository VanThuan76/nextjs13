...
const Header = ({activeItem}: Props) => {
    ...
    return (
    <div className={`w-full p-5 border-b min-h-[60px] border-b-[#ffffff32] transition-opacity &{active && 'fixed top-0 left-0 bg-cyellow-600 z-[9999]'`}>
        <div className='hidden md:w-[90%] mx-auto md:flex items-center justify-between'>
            <div>
                <Link href={'/'}>
                    <h1 className='font-Inter text-3xl cursor-pointer'>
                        <span className='text-white'>Magic</span>Post
                    </h1>
                </Link>
            </div>
            <div className='flex'>
            ...
            </div>
            {
                open && (
                    <div className='fixed md:hidden w-full h-screen top-0 left-0 z-[99999] bg-[unset]'
                        onClick={handleClose}
                        id='screen'>
                        <div className='fixed bg-cyellow-300 h-screen top-0 right-0 w-[45%] z-[9999]'>
